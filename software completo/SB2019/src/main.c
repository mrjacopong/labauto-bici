...
PIDSt_Type PID_P_curr_2; // pid control for steering current
// AGGIUNTO DA LOLLO
PIDSt_Type PID_P_vel_1;  // pid control for traction velocity
PIDSt_Type PID_P_curr_1; // pid control for traction current
...
/*-----------------------------------------------------------------------------------------------------------------------------*/
	...
	PWM_Init(3,12,&m1); // PWM motor 1 (trazione) JN2-PIN 23  ->  PE2
	PWM_Init(4,12,&m2); // PWM motor 2 (sterzo)   JN2-PIN 22  ->  PE1

	/* Inizializzazione dei parametri per il controllo PID in coppia- MANUBRIO*/
	init_pid(&PID_P_curr_2,SET_PARAM_PID_TORQUE_2);
	/* Inizializzazione dei parametri per il controllo PID in coppia- TRAZIONE*/
	init_pid(&PID_P_curr_1,SET_PARAM_PID_TORQUE_1);

	/* Inizializzazione dei parametri per il controllo PID in velocita' - TRAZIONE */
	init_pid(&PID_P_vel_1,SET_PARAM_PID_SPEED);
...
/*-----------------------------------------------------------------------------------------------------------------------------*/
	...
	while(1){
		...
		/* Riferimento per pid in m/s - TRAZIONE */
		PID_P_vel_1.uc =(float) V_ms / (float) R_bike;
		...
		/* Assegnazione della velocita letta dall'encoder all'uscita per il calcolo dell'errore del PID per la velocita */
		PID_P_vel_1.y = enc_1.speed_in_rad_per_sec;
		/* Calcolo dell'uscita del pid per il controllo in cascata */
		calcPID(&PID_P_vel_1);
		...
		PID_P_curr_1.uc = PID_P_vel_1.output;
...
/* LCD DEL MOTORE DI TRAZIONE
		//Stampa a video ogni 1s.
				if(!(get_ms()%1000)){
					...
					sprintf((char *)lcd_buffer, "RIF: %.3f",PID_P_curr_2.uc);
					lcd_display(LCD_LINE5, lcd_buffer);
		...
		//sprintf((char *)lcd_buffer, "S.C.:=%.4f",SENS_S.sens_curr_2);
		//lcd_display(LCD_LINE4, lcd_buffer);

		sprintf((char *)lcd_buffer, "RIF=%.1f",PID_P_curr_2.uc);
		lcd_display(LCD_LINE5, lcd_buffer);
