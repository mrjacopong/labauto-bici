 ...
 *******************************************************************************************************************************/
    ...
    float vettore_Vout_1[lengthADb];   //vettore di uscita per ogni sensore, dato che abbiamo tre sensori ci sono 3 vettori formati da lenght elementi, cioè lenght misurazioni date da lenght
    float vettore_Vout_2[lengthADb];
	...
	float filtraggio (float read,int contatore,float vettore_Vout[])
	    {
	      	...
	      	float somma = 0;
	      	float Vout;
	      	if(contatore<lengthADb) //Ci dice se abbiamo raggiunto il limite lenght di valori
	      					{
	      					...
	      					else // Eseguiamo lo shift, dato che abbiamo già esaminato lenght valori
	      					{
	      						...
	      						vettore_Vout[lengthADb-1] = read;
	      						somma += vettore_Vout[lengthADb-1]*(i+1);
	...
	***********************************************************************************************/
	...
	{
    	float read_register, Vout_1; /*read_register è la variabile nella quale sono inseriti i valori
    												   di tensione letti dai registri*/
        // AD.ADDRC/E/F sono i registri dove vengono “salvati” i valori letti dal sensore di corrente

				read_register = ((AD.ADDRC*VMAX)/MAX_COUNT); /*legge il risultato del registro per la porta AN2
